...
const trainingExerciseSchema = z.object({
  ...
  block_name: z.enum(["Warm-up", "Hauptteil", "Cool-down"]),
  sets: z.string().min(1).max(80),
  reps: z.string().min(1).max(80),
  load_text: z.string().max(120).optional(),
  rpe_target: z.string().min(1).max(80),
  rest_seconds: z.coerce.number().int().nonnegative(),
  tempo: z.string().max(120).optional(),
  technical_notes: z.string().min(1).max(1000),
  today_focus: z.string().min(1).max(1000),
  ...
  for (const blockName of ["Warm-up", "Hauptteil", "Cool-down"]) {
    if (!plan.exercises.some((exercise) => exercise.block_name === blockName)) {
      ctx.addIssue({
        ...
        path: ["exercises"],
        message: `Mindestens eine Uebung im Block ${blockName} ist bei should_train=true Pflicht.`
      });
    }
  }
});
